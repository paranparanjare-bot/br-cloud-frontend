...
import { Search, Folder, FileText, Cloud, Home, ChevronLeft, RefreshCw, LayoutGrid, List, Maximize, Minimize2, User, ArrowUp, ArrowDown } from 'lucide-react';

// Mengambil URL dari file .env (Vite menggunakan import.meta.env)
...
export default function Dashboard() {
  ...
  const loadAccounts = async () => {
    ...
    } catch (e) { console.error("Error load accounts:", e); }
  };
  ...
  const loadFiles = async (email, folderId = 'root') => {
    ...
    } catch (e) { console.error("Error load files:", e); }
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-800 font-sans">
      <aside className="w-64 bg-white border-r p-6 flex flex-col shadow-sm">
        <div className="flex items-center mb-10 text-blue-600 font-bold text-xl uppercase italic"><Cloud className="mr-2"/> BR Drive</div>
        <div className="space-y-2 flex-1 overflow-y-auto">
          ...
          <button onClick={() => fetch(`${API_BASE_URL}/api/auth/google`).then(r => r.json()).then(d => window.location.href=d.url)}
            className="w-full p-3 rounded-xl text-xs font-bold border-2 border-dashed border-slate-200 text-slate-400 hover:text-blue-500 transition-all">+ Akun</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="p-10 overflow-y-auto">
          {isLoading ? (
            <div className="text-center py-40 animate-pulse text-blue-500 font-black tracking-widest uppercase">Sinkronisasi Cloud...</div>
          ) : (
